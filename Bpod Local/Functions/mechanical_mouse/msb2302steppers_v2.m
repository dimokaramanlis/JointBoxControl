classdef msb2302steppers
    ...
    methods
        ...
        function error = setMotorTopFrequency(obj, motorNb, frequency, stepMode)
            ...
            if ~isValid
                disp('[ERROR] setMotorTopFrequency: [topspeed reg] Invalid ack');
            end

            % Get the actual motors configuration from device
            devConfigValue = obj.getDeviceRegisterValue(xstep_config_reg);
            % Reset b0 & b1 to unipolar
            devConfigValue = bitand(devConfigValue, bitcmp(0x03, 'uint8'));


            % Adjust step mode in config register
            if stepMode == 0
                devConfigValue = bitor(devConfigValue, uint8(0x01));
            else
                devConfigValue = bitor(devConfigValue, uint8(0x02));
            end

            % Envoi d'une requête d'écriture pour obtenir
            setRegistersSeq = [0x00, obj.i2cAdr, xstep_config_reg, devConfigValue];
            disp(sprintf('Set xstep_config_reg to: %d ', devConfigValue));
            obj.writeSerialData(setRegistersSeq); % Utilisation de la méthode définie ici
            ...
            if ~isValid
                disp('[ERROR] setMotorTopFrequency: [config reg] Invalid ack');
            end              
        end
