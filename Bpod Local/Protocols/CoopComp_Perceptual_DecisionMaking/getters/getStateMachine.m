...
times.RewardDelay = S.GUI.RewardDelay;     % BA
...
if numel(mousesetting)==2  
   ...
   if times.CooperationTimeout > times.DecisionTimeout   %No cooperation trials
        sma = getTwoMiceStateMachine(choices,actions,times,conditions);
    else                                                  %Cooperation trials
        
         conditions.MouseTerminate  = {'GlobalTimer2_End','customExit'};
         if S.GUI.Terminate           
                conditions.MouseTerminate = {choices.m1IncorrectChoice,'M1WrongBothPunished',...
                                           choices.m2InCorrectChoice,'M2WrongBothPunished',...                                 
